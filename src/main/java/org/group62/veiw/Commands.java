...
public enum Commands {
    ...
    USER_STAY_LOGGED_IN("^\\s*user\\s+login(?<args>(?=.+(-u)\\s+((?<username>\"(.*\\S+\\s+.*)\"|.+?(?=\\s+-p|" +
            ...
            "|.+)))(?=.+(--stay-logged-in)).+)\\s*$"),
    FORGOT_MY_PASSWORD("^\\s*forgot\\s+my\\s+password\\s+(-u)\\s+(?<username>\\S+)\\s*$"),
    ...
    SET_STATE("^\\s*set(?<args>(?=.+(-x)\\s+(?<xCoordinates>\\d+))(?=.+(-y)\\s+(?<yCoordinates>\\d+))(?=.+(-s)" +
            "\\s+(?<state>\"(.*\\S+\\s+.*)\"|.+?(?=\\s+-y|\\s+-s|\\s+-x)|.+)).+)\\s*$"),
    ATTACK_COORDINATES("^\\s*attack\\s+-e\\s+(?<enemyx>\\d+)\\d+(?<enemyy>\\d+)\\s*$"),
    ATTACK("^\\s*attack(?<args>(?=.+(-x)\\s+(?<xCoordinates>\\d+))(?=.+(-y)\\s+(?<yCoordinates>\\d+)).+)\\s*$"),
    POUR_OIL("^\\s*pour\\s+oil\\s+-d\\s+(?<direction>\"(.*\\S+\\s+.*)\"|.+)\\s*$"),
    DIG_TUNNEL("^\\s*dig\\s+tunnel(?<args>(?=.+(-x)\\s+(?<xCoordinates>\\d+))" +
            "(?=.+(-y)\\s+(?<yCoordinates>\\d+)).+)\\s*$"),
    BUILD("^\\s*build\\s+-q\\s+(?<equipmentName>\"(.*\\S+\\s+.*)\"|.+)\\s*$"),
