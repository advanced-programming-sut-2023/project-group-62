...
public class ChangeSloganController {
    public TextField newSlogan;

    public void sloganChange(MouseEvent mouseEvent) {
        ProfileMenuController profileMenuController = new ProfileMenuController();
        String output = profileMenuController.changeSlogan(newSlogan.getText());
        if (output.equals("change slogan was successful")){
            Alert informationAlert = new Alert(Alert.AlertType.INFORMATION);
            informationAlert.setTitle("Susses");
            informationAlert.setHeaderText("slogan change");
            informationAlert.setContentText(output);
            informationAlert.showAndWait();
        } else {
            Alert errorAlert = new Alert(Alert.AlertType.ERROR);
            errorAlert.setTitle("Error");
            errorAlert.setHeaderText("change slogan failed");
